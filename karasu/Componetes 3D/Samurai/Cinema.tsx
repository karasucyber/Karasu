...
const Samurai = React.memo(function Samurai(props: any) {
  const colorTexture = useTexture('Robe2_normal.jpeg');
  const { scene } = useGLTF('/Samurai.gltf');
  return <primitive object={scene} map={colorTexture} {...props} />;
});

const VideoText = React.memo(function VideoText(props: any) {
...
const Ground = React.memo(function Ground() {
  ...
  useEffect(() => {
    // Carregue texturas aqui de forma assíncrona, se necessário
