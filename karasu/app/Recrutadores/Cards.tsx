...
const Scene: React.FC = ({ children, ...props }) => {
...
const Cards: React.FC<CardsProps> = ({ category, data, from = 0, len = Math.PI * 2, radius = 5.25, onPointerOver, onPointerOut, ...props }) => {
  const [hovered, hover] = useState<number | null>(null)
...
const Card: React.FC<CardProps> = ({ url, active, hovered, ...props }) => {
  const ref = useRef<THREE.Group>(null)
  useFrame((state, delta) => {
...
const ActiveCard: React.FC<ActiveCardProps> = ({ hovered, ...props }) => {
  const ref = useRef<THREE.Group>(null)
